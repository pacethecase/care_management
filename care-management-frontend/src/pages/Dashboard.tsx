...
let filteredUsers = allUsers;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      {showOrgsModal && user?.has_global_access && (
        <OrganizationsModal
...
const SummaryCard = ({ title, count, onClick }: SummaryCardProps) => (
  <div
    ...
    className="
      ...
      hover:shadow-md transition cursor-pointer flex flex-col items-start
    "
  >
    <h3 className="text-lg font-semibold text-gray-800">{title}</h3>

    <p className="text-4xl font-bold text-gray-900 mt-2 mb-4">
      {count ?? "-"}
    </p>

    <button
      className="
        px-4 py-2 text-sm bg-[var(--prussian-blue)] 
