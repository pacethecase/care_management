// src/pages/ArchivedPatients.tsx
...
const ArchivedPatients = () => {
  ...
  const { archivedPatients, archivedLoading, archivedError, searchResults } =
  ...
  useEffect(() => {
    const delay = setTimeout(() => {
      ...
      if (trimmed) {
        dispatch(searchPatients({ query: trimmed, status: "archived", start: start || undefined, end: end || undefined }));
      } else {
        dispatch(fetchArchivedPatients({ start: start || undefined, end: end || undefined }));
      }
    }, 300);
    ...
    <div className="flex flex-col min-h-screen bg-[var(--bg-light)] text-[var(--text-dark)]">
      ...
      <div className="p-6">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-4 gap-4">
          <div>
            <h2 className="text-2xl font-bold text-red-600">Archived Patients</h2>
            {!archivedLoading && (
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-8 flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="relative w-full sm:w-64">
            <input
              ...
              className="w-full pl-4 pr-4 py-2 rounded-full border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:outline-none shadow-sm"
            />
          </div>
        </div>
        ...
        {!archivedLoading && dataToDisplay.length === 0 && (
          <p className="text-gray-500">
            {start || end ? "No archived patients in this date range." : "No archived patients found."}
          </p>
        )}
