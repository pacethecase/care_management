// src/pages/LengthOfStayReport.tsx
...
const LengthOfStayReport = () => {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <html><head><title>Length of Stay Report</title>
      ...
      <style>
        ...
        .logo { height: 80px; display: block; margin: 0 auto 10px; }
        h1 { text-align: center; font-size: 26px; color: #003049; margin-bottom: 10px; }
        .no-break, .card, .chart-container { page-break-inside: avoid; break-inside: avoid; }
      </style></head>
      <body>
  ...
  const renderCard = (label: string, values: any) => {
    const color = algoColors[label] || "var(--prussian-blue)";
    return (
      <div className="p-5 bg-white rounded-xl shadow-sm border border-gray-200" style={{ borderLeft: `5px solid ${color}` }}>
        ...
        <div className="space-y-1 text-sm text-gray-700">
          ...
          <p><span className="font-medium">Total LOS:</span> {values.totalDisplay ?? `${values.totalDays}d`}</p>
          <p><span className="font-medium">Average LOS:</span> {values.avgDisplay ?? `${values.avgDays}d`}</p>
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200 mb-8">
          <div className="flex flex-wrap gap-6">
            ...
            <label className="flex items-center gap-2 mt-5 text-sm">
              <input type="checkbox" checked={showUnique}
                onChange={() => setShowUnique(p => !p)} />
              Show Unique Total
            </label>
        ...
        {data && (
          <div id="los-content">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6 no-break">
              ...
              {showUnique && data.totalUnique && renderCard("Unique Patients", {
                count:        data.totalUnique.count,
                totalDisplay: data.totalUnique.totalDisplay,
                avgDisplay:   data.totalUnique.avgDisplay,
                cost:         data.totalUnique.cost,
              })}
            </div>
            <div className="chart-container">
              <LOSDashboardChart
                data={[
                  ...
                  ...(showUnique && data.totalUnique ? [{
                    workflow:  "Unique Patients",
                    totalDays: data.totalUnique.totalDays,
                    totalCost: data.totalUnique.cost,
                  }] : []),
                ]}
