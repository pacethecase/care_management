// src/components/LOSDashboardChart.tsx
import React from "react";
import {
  BarChart, Bar, XAxis, YAxis, Tooltip,
  CartesianGrid, ResponsiveContainer, Legend, LabelList, Cell,
} from "recharts";
...
const COLORS: Record<string, string> = {
  ...
  LTC:              "var(--algo-ltc)",
  // FIX: distinct color for the unique total bar
  "Total (Unique)": "var(--prussian-blue)",
};
    ...
    <div className="grid gap-10 md:grid-cols-2 mt-10">
          ...
          <BarChart data={data} margin={{ top: 40, right: 30, left: 0, bottom: 0 }}>
            ...
            <Bar dataKey="totalDays" name="Days">
              ...
              {data.map((entry, index) => (
                <Cell
                  key={`cell-days-${index}`}
                  fill={COLORS[entry.workflow] ?? "#003049"}
                />
              ))}
            </Bar>
          </BarChart>
      ...
      <div className="p-4 bg-white rounded-xl shadow-md">
        ...
        <p className="text-sm text-gray-500 italic mt-2">
          Total cost is calculated using Individual Hospital Daily Rates and/or
          National Hospitalization Average of $2883 if not available.
        </p>
      </div>
